...
public class OppoPush {
    ...
    public void push_aw_chat(PushMessage pushMessage) {
        ...
        Result result = null;
        try {
            Notification notification = getNotification_aw_chat(pushMessage); //创建通知栏消息体
    ...
    private Notification getNotification_aw_chat(PushMessage pushMessage) {
        ...
        if (pushMessage.pushMessageType == PushMessageType.PUSH_MESSAGE_TYPE_FRIEND_REQUEST) {
            if (StringUtils.isEmpty(pushMessage.senderName)) {
                ...
                notification.setTitle(pushMessage.senderName + " 请求加您为好友");
            }
            ...
            if (StringUtils.isEmpty(pushMessage.senderName)) {
                ...
                notification.setTitle(pushMessage.senderName);
            }
            notification.setContent(pushMessage.pushContent);
        } else {
            if (StringUtils.isEmpty(pushMessage.senderName)) {
            ...
            notification.setContent(pushMessage.unReceivedMsg==2?"邀请您语音通话":"邀请您视频通话");
        }
